function innerMapTester(){
	const descString = parent.document.getElementById("descwr").value;
...
function reloadBody(){
	document.getElementById("codebody").innerHTML = codeStringChanger(parent.document.getElementById("codewr").value);
...
function codeStringChanger(rawString){
	let modifiedString = rawString.replace(/\&/g,"&amp;").replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;").replace(/\'/g,"&#39;");
	 modifiedString = modifiedString.replace(/\[desc (.*?)\]/g,'<input type="button" value="Description - $1" onclick="createWindow(\'$1\')"/>');
...
function getStartRow(tagName){
	const codeString = parent.document.getElementById("codewr").value;
...
function iframePreviewSet(){

	const codeText = window.parent.document.code.codewr.value;
	...
	console.log("CB:"+document.getElementById("codebody"));
