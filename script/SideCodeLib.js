...
function placeTag(){
  ...
  //console.log("tagName1 : "+tagName);
	if(tagName==nullTx){
		...
		autoInsertIndex++;
		tagName='autoInsert_'+autoInsertIndex;
		console.log("aI:"+autoInsertIndex);
	}
	const fomrerTag = "[desc "+tagName+"]\n";
	const latterTag = "\n\n[/desc "+tagName+"]\n";

	codeUndoStacker(1);
	stringInserter(document.code.codewr,fomrerTag,latterTag);
	...
	*/
	...
	descUndoStacker(1);
	stringInserter(document.desc.descwr,fomrerTag,latterTag);
	/*
...
document.addEventListener('keydown', (event) => {
  //console.log("=============Frame Reset=============");
	...
	//というか全角判定なら全部229らしい(改行は全角ないから入らないけど)
	cutEvent=0;
	pasteEvent=0;
	prevRemoveFlag = removeFlag;
	removeFlag = 1*(event.keyCode===8)+2*(event.keyCode===46)+4*(cutEvent===1);
	//console.log("rF:"+removeFlag+"prF:"+prevRemoveFlag);
	if(event.ctrlKey){
...
document.addEventListener('input', (event) => {
	...
	const bracketsEnd=[")","}",">","]"];
	if(!removeFlag && cutEvent!=1){
		if(keywordJudger(inputCharReader(0))!=0){
			stackExecutor();
		}

		if(Math.floor(keywordJudger(inputCharReader(0))/10)==1){
			//console.log("before:"+document.activeElement.selectionStart);
			const carsolPos = document.activeElement.selectionStart;
			stringInserter(document.activeElement,"",bracketsEnd[keywordJudger(inputCharReader(0))-10]);
			document.activeElement.setSelectionRange(carsolPos,carsolPos);
			console.log("after :"+document.activeElement.selectionStart);
		}
	}

/*
	if(removeFlag==1){
		if(Math.floor(keywordJudger(inputCharReader(0)))==2){
			null;
		}
	}
	*/
...
function inputCharReader(offset){
	const inputArea = document.activeElement;//.id;
	if(inputArea.id=="TagName"){return -2;}
	//console.log("S:"+inputArea.selectionStart+" E:"+inputArea.selectionEnd);
	let inputChar = inputArea.value.substr(inputArea.selectionEnd-1+offset,1);
...
function keywordJudger(char){
	...
	for(i=0; i<keywords.length; i++){
		...
		else if(i<bracketsStart.length&&char==bracketsStart[i]){return 10+i;}
		else if(i<bracketsEnd.length&&char==bracketsEnd[i]){return 20+i;}
	}
	return 0;
}
...
function previewCreator(){
	//let child = window.open("code_template.html");
	//let child = window.open("code_template_tester.html");
	let child = window.open("code_template_HL.html");
}

function stringInserter(element,formerString,latterString){
...
function previewSet(){
	if(!previewFlag){return -1;}

	const codeText = document.code.codewr.value;
	let modifiedString = codeText.replace(/\&/g,"&amp;").replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;").replace(/\'/g,"&#39;");
	 ...
	 console.log("cT:"+modifiedString);

	const iframe = document.getElementById("previewMain");
	//iframe.innerHTML = modifiedString;
	console.log(iframe.contentWindow.document);
	console.log("CB:"+iframe.contentWindow.document.getElementById("codebody"));
	//document.getElementById("codebody").innerHTML = modifiedString;
}
